...
const tabItems = computed(() => [
  { label: t('players.onlineTab'), icon: 'i-lucide-wifi', slot: 'online' as const },
  { label: t('players.whitelist'), icon: 'i-lucide-shield-check', slot: 'whitelist' as const },
  { label: t('players.blacklist'), icon: 'i-lucide-ban', slot: 'blacklist' as const },
  { label: t('players.admins'), icon: 'i-lucide-crown', slot: 'admins' as const }
])
...
async function kickPlayer(guid: string) {
  ...
  try {
    ...
    toast.add({ title: t('players.playerKicked'), color: 'success' })
    setTimeout(() => refreshPlayers(), 1000)
  } catch {
    toast.add({ title: t('players.kickFailed'), color: 'error' })
  } finally {
    kicking.value = null
  }
}
...
async function banPlayer(guid: string) {
  ...
  try {
    ...
    toast.add({ title: t('players.playerBanned'), color: 'success' })
    refreshBlacklist()
  } catch {
    toast.add({ title: t('players.banFailed'), color: 'error' })
  } finally {
    banning.value = null
  }
}
...
async function addToList(listType: 'whitelist' | 'blacklist' | 'admins') {
  ...
  try {
    ...
    toast.add({ title: t('players.addedTo') + ' ' + listType, color: 'success' })
    newGuid.value = ''
    if (listType === 'whitelist') refreshWhitelist()
    else if (listType === 'blacklist') refreshBlacklist()
    else refreshAdmins()
  } catch {
    toast.add({ title: t('players.addFailed'), color: 'error' })
  }
}
...
async function removeFromList(listType: 'whitelist' | 'blacklist' | 'admins', guid: string) {
  ...
  try {
    ...
    toast.add({ title: t('players.removedFrom') + ' ' + listType, color: 'success' })
    if (listType === 'whitelist') refreshWhitelist()
    else if (listType === 'blacklist') refreshBlacklist()
    else refreshAdmins()
  } catch {
    toast.add({ title: t('players.removeFailed'), color: 'error' })
  }
}

// Auto-refresh online players (client-only)
let refreshInterval: ReturnType<typeof setInterval>
onMounted(() => { refreshInterval = setInterval(() => refreshPlayers(), 10000) })
  ...
  <UDashboardPanel>
    <template #header>
      <UDashboardNavbar :title="t('players.title')">
        ...
        <template #right>
          <UBadge :label="(players?.length || 0) + ' ' + t('players.online')" :color="players?.length ? 'success' : 'neutral'" variant="subtle" />
        </template>
      </UDashboardNavbar>
    </template>
        ...
        <UTabs :items="tabItems" class="w-full">
              ...
              <UCard v-for="player in players" :key="player.guid">
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <p class="text-xs text-(--ui-text-muted)">{{ player.car }}{{ player.nation ? ` · ${player.nation}` : '' }}</p>
                  </div>
          ...
          <template #whitelist>
            <div class="pt-4 space-y-4">
              <div class="flex gap-2">
                <UInput v-model="newGuid" :placeholder="t('players.steamGuidPlaceholder')" class="flex-1" />
                <UButton :label="t('players.add')" icon="i-lucide-plus" @click="addToList('whitelist')" />
              </div>
          ...
          <template #blacklist>
            <div class="pt-4 space-y-4">
              <div class="flex gap-2">
                <UInput v-model="newGuid" :placeholder="t('players.steamGuidPlaceholder')" class="flex-1" />
                <UButton :label="t('players.add')" icon="i-lucide-plus" @click="addToList('blacklist')" />
              </div>

              <div v-if="!blacklist?.length" class="text-(--ui-text-muted) text-center py-8">
                {{ t('players.blacklistEmpty') }}
              </div>
          ...
          <template #admins>
            <div class="pt-4 space-y-4">
              <div class="flex gap-2">
                <UInput v-model="newGuid" :placeholder="t('players.steamGuidPlaceholder')" class="flex-1" />
                <UButton :label="t('players.add')" icon="i-lucide-plus" @click="addToList('admins')" />
              </div>
